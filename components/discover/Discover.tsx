import AdvancedFilterOverlay from "./AdvancedFilterOverlay";
import type { DB_TYPE } from "./discover-types";
import { DiscoverConfig, DiscoverConfigProvider } from "./discover-config";
import DiscoverInfluencer from "./DiscoverInfluencer";
...
import { cleanFilters, hasMeaningfulFilters } from "./utils/filter-utils";
import {
    GUIDE_TOUR_MOBILE,
...
const DiscoverComponent = ({
    ...
    const { managerId, selectedBrand, updateBrand } = config;
    ...
    const guideTourShownKey =
        managerId
            ? `discover-guide-tour-shown-${managerId}-${xl ? "web" : "mobile"}`
            : null;
    ...
    const { AppLayout } = config.components;
    ...
    if (!managerId || !selectedBrand || !selectedBrand.id)
        return (
            <SlowLoader
                messages={[
                    "Loading brand information...",
    ...
    return (
        <DiscoverConfigProvider value={config}>
            <DiscoveryProvider
                value={{
                    selectedDb,
                    setSelectedDb,
